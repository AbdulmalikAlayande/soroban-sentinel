...
The codebase strictly follows a one-way dependency rule where the `core/` business logic never depends on the `commands/` CLI layer.

```mermaid
graph TD
    commands["src/commands/<br/>(CLI wrappers)"] --> core["src/core/<br/>(Business Logic)"]
    commands --> daemon["src/daemon/<br/>(Loop)"]
    commands --> alerts["src/alerts/<br/>(Delivery)"]
    commands --> db["src/db/<br/>(Storage)"]

    daemon --> core
    daemon --> alerts
    daemon --> db

    core --> rpc["src/rpc/<br/>(Stellar Network)"]
    core --> alerts
    core --> db
