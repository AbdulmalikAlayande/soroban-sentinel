...
export interface ContractGroupMember {
    ...
    group_id: number;
    contract_id: string;
    /** ISO-8601 timestamp of when the contract was added to the group. */
    added_at: string;
}
...
/**
 * Return a group by its unique name, or `undefined` if it does not exist.
 */
export function getGroupByName(
    db: Database.Database,
    name: string,
): ContractGroup | undefined {
    return db.prepare("SELECT * FROM contract_groups WHERE name = ?")
        .get(name) as ContractGroup | undefined;
}
...
export function getAllGroups(
    ...
    return db.prepare("SELECT * FROM contract_groups ORDER BY name ASC")
        .all() as ContractGroup[];
}

/**
 * Return the membership rows for the given group (contract_id + added_at),
 * ordered by when each contract was added.
 */
...
/**
 * Return true when the given contract is a member of the given group.
 */
export function isContractInGroup(
    db: Database.Database,
    groupId: number,
    contractId: string,
): boolean {
    const row = db.prepare(`
        SELECT 1 FROM contract_group_members
        WHERE group_id = ? AND contract_id = ?
        LIMIT 1
    `).get(groupId, contractId) as { 1: number } | undefined;
    return row !== undefined;
}
