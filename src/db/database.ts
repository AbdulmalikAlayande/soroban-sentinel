...
export function getDatabase(customPath?: string): Database.Database {
    ...
    const migrations = [
        ...
        // issue #394 — timestamp of when a contract joined a group
        `ALTER TABLE contract_group_members ADD COLUMN added_at DATETIME DEFAULT CURRENT_TIMESTAMP`,
    ];
...
function migrateAlertConfigsChannelTypeCheck(db: Database.Database): void {
    ...
    `);
    db.exec(`
        INSERT INTO alert_configs_new (id, contract_id, channel_type, channel_target, threshold_ledgers, webhook_secret, quiet_hours_start, quiet_hours_end, quiet_hours_timezone, enabled, created_at)
        SELECT id, contract_id, channel_type, channel_target, threshold_ledgers, webhook_secret, quiet_hours_start, quiet_hours_end, quiet_hours_timezone, enabled, created_at
        FROM alert_configs
    `);
...
function relaxChannelTypeChecks(db: Database.Database): void {
    ...
    if (hasEnumCheck("alert_configs")) {
        ...
        `);
        db.exec(`
            INSERT INTO alert_configs_relaxed (id, contract_id, channel_type, channel_target, threshold_ledgers, webhook_secret, quiet_hours_start, quiet_hours_end, quiet_hours_timezone, enabled, created_at)
            SELECT id, contract_id, channel_type, channel_target, threshold_ledgers, webhook_secret, quiet_hours_start, quiet_hours_end, quiet_hours_timezone, enabled, created_at
            FROM alert_configs
        `);
        db.exec(`DROP TABLE alert_configs;`);
