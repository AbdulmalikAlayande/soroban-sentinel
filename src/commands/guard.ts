...
export function registerGuardCommand(program: Command): void {
    ...
    guard
        ...
        .option("--json", "Output machine-readable JSON")
        .action(async (contractId: string | undefined, options: { json?: boolean; tag?: string; preset?: string; targetTtl?: string; threshold?: string; keypair?: string; keypairEnv?: string; keypairVault?: string; autoExtend?: boolean; predictive?: string; dryRun?: boolean; disable?: boolean } = {}) => {
            try {
                ...
                if (options.autoExtend) {
                    ...
                    if (options.json) {
                        printOutput({ success: true, contractId, mode: "auto-extend", policy: { enabled: true, target_ttl_ledgers: targetTTL, extend_when_below_ledgers: threshold, keypair_source: keypairSource, keypair_public: kp.publicKey(), predictive_cycles: predictiveCycles } }, true);
                        return;
                    }
