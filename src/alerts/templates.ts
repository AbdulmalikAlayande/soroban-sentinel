...
Handlebars.registerHelper("localeString", (num) =>
  typeof num === "number" ? num.toLocaleString() : num,
);
Handlebars.registerHelper("json", (obj) => JSON.stringify(obj));
Handlebars.registerHelper("escapeJson", (str) =>
  typeof str === "string" ? JSON.stringify(str).slice(1, -1) : str,
);
Handlebars.registerHelper("escapeMarkdown", (text) =>
  typeof text === "string"
    ? text.replace(/[_*[\]()~`>#+=|{}.!\\-]/g, "\\$&")
    : text,
);
...
export function getTemplateContext(event: AlertEvent) {
  ...
  return {
    ...
    severityLevel,
    entryLabel,
    severityEmoji: isResolved
      ? "✅"
      : event.severity === "critical"
        ? "🔴"
        : "⚠️",
    isTTLAlert:
      event.type === "threshold_crossed" || event.type === "alert_resolved",
    isResourceAlert,
    isResolved,
    isCritical: event.severity === "critical",
    isWarning: event.severity === "warning",
    isInfo: event.severity === "info",
    currentUsageFormatted: isResourceAlert
      ? event.resource.currentUsage.toLocaleString()
      : "",
    limitFormatted: isResourceAlert
      ? event.resource.limit.toLocaleString()
      : "",
    currentRemainingLedgersFormatted:
      "threshold" in event
        ? event.threshold.currentRemainingLedgers.toLocaleString()
        : "",
    configuredLedgersFormatted:
      "threshold" in event
        ? event.threshold.configuredLedgers.toLocaleString()
        : "",
...
export function renderAlertTemplate(
  ...
  if (!templatesPath) {
    try {
      ...
      if (
        "templatesPath" in config &&
        typeof config.templatesPath === "string"
      ) {
