...
export async function sendEmailAlert(target: string, event: AlertEvent): Promise<void> {
    ...
    try {
        ...
        // Sanitize: strip password if it somehow appears in the error, in
        // both the message we throw and the caught error itself — `cause`
        // preserves the original object, and an unredacted password could
        // otherwise leak through it (e.g. via a stack trace) even though
        // the top-level message here is clean.
        if (err instanceof Error && err.message.includes(smtp.pass)) {
            err.message = err.message.replaceAll(smtp.pass, "[REDACTED]");
        }
        const sanitized = message.includes(smtp.pass) ? message.replaceAll(smtp.pass, "[REDACTED]") : message;
        throw new Error(`Email delivery failed: ${sanitized}`, { cause: err });
    }
