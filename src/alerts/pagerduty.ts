...
function buildSummary(event: AlertEvent): string {
    ...
    if (event.type === "budget_exhausted") {
        return `Sorokeep alert: ${contractDisplay} budget exhausted — auto-extension blocked for billing cycle ${event.budget.billingCycle}.`;
    }
...
function buildPayload(event: AlertEvent): unknown {
    const isTrigger = event.type === "threshold_crossed" || event.type === "resource_alert" ||
        event.type === "state_changed" || event.type === "budget_exhausted";
    const component = event.type === "resource_alert"
        ? "resource_monitor"
    ...
    const cls = event.type === "resource_alert"
        ? `resource:${event.resource.type}`
        : event.type === "state_changed"
            ? `state:${event.diff.diffType}`
            : event.type === "budget_exhausted"
                ? `budget:${event.budget.billingCycle}`
                : `threshold:${event.threshold.configuredLedgers}`;
    return {
        routing_key: "",
        event_action: isTrigger ? "trigger" : "resolve",
