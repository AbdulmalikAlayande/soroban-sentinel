            ...
            try {
                ...
                if (budget) {
                    ...
                    if (budget.spent_xlm + estimatedFeeXlm > budget.limit_xlm) {
                        // Fire a budget_exhausted alert through the real dispatcher (best-effort)
                        // before re-throwing so the extension is still blocked.
                        const alertConfigs = getAlertConfigsForContract(db, contract.id);
                        const budgetEvent = buildBudgetExhaustedAlertEvent({
                            contractId: contract.id,
                            contractName: (contract as any).name ?? null,
                            network,
                            billingCycle,
                        ...
                        for (const cfg of alertConfigs) {
                            deliverSingleAlert(
                                ...
                                logger.warn(
                                    `Budget-exhausted alert delivery failed for config ${
                                        cfg.id
