...
Manage alert configurations. Supports subcommands for add, list, remove, enable, disable, test, test-all, history, stats, and channels.
...
| `--type <type>` | Notification channel type — run `sorokeep alerts channels` for the full list (e.g. `webhook`, `slack`, `discord`, `telegram`, `pagerduty`, `email`) |
...
Email alerts are supported and delivered via SMTP. Configure the SMTP server (`host`/`port`/`user`/`pass`) in `~/.sorokeep/config.yaml`, then create an alert with `sorokeep alerts add --contract <id> --type email --channel <recipient> --threshold <ledgers>`.
