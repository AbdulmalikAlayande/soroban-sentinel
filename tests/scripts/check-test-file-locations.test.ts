...
describe("check-test-file-locations", () => {
  ...
  it("exits non-zero on the actual repo because src/alerts/*.test.ts exist", () => {
    const repoRoot = resolve(__dirname, "../..");
    const { exitCode, stdout, stderr } = runScript(repoRoot);
    // The 10 misplaced src/alerts/*.test.ts files must cause failure
    expect(exitCode).not.toBe(0);
    const combined = stdout + stderr;
    expect(combined).toMatch(/src\/alerts\//);
  });
});
