...
describe("AlertConfig Targets Fan-out", () => {
    ...
    it("should remove a target from an alert_config", () => {
        addTargetToAlertConfig(db, alertConfigId, "slack", "C12345");
        addTargetToAlertConfig(db, alertConfigId, "pagerduty", "pd-routing-key");
        expect(getAlertConfigTargets(db, alertConfigId)).toHaveLength(2);

        removeTargetFromAlertConfig(db, alertConfigId, "slack", "C12345");

        const remaining = getAlertConfigTargets(db, alertConfigId);
        expect(remaining).toHaveLength(1);
        expect(remaining[0]?.channel_type).toBe("pagerduty");
    });
