...
import { insertContract, insertDigestConfig } from "../../src/db/repositories.js";
...
describe("digest loop scheduling (AC #1)", () => {
    ...
    it("delivers the digest via deliverSingleAlert for each enabled config", async () => {
        insertDigestConfig(db, {
            network: "testnet",
            channel_type: "webhook",
    ...
    it("does not stop the daemon if digest delivery fails", async () => {
        ...
        mockDeliverSingleAlert.mockRejectedValueOnce(new Error("network timeout"));
