...
import {
    getExtensionCosts,
    calculateFeeAdjustedProjection,
    calculateTrendAdjustedProjection,
    getMultiPeriodCosts,
} from "../../src/core/costs.js";
...
describe("calculateTrendAdjustedProjection", () => {
    it("returns a rising trend when recent fees are higher than earlier fees", () => {
        const recentBaseFees = [100, 100, 100, 100, 200, 200, 200, 200];

        const result = calculateTrendAdjustedProjection(1.0, 30, recentBaseFees);

        expect(result.flatProjectedCostXlm).toBe(1);
        expect(result.trendAdjustedProjectedCostXlm).toBeGreaterThan(result.flatProjectedCostXlm);
        expect(result.trendMultiplier).toBeGreaterThan(1);
        expect(result.feeTrend).toBe("rising");
    });

    it("returns a stable trend when fee history is flat", () => {
        const recentBaseFees = [100, 100, 100, 100, 100, 100, 100, 100];

        const result = calculateTrendAdjustedProjection(1.0, 30, recentBaseFees);

        expect(result.trendAdjustedProjectedCostXlm).toBeCloseTo(result.flatProjectedCostXlm, 7);
        expect(result.trendMultiplier).toBeCloseTo(1, 7);
        expect(result.feeTrend).toBe("stable");
    });

    it("returns a falling trend when recent fees are lower than earlier fees", () => {
        const recentBaseFees = [200, 200, 200, 200, 100, 100, 100, 100];

        const result = calculateTrendAdjustedProjection(1.0, 30, recentBaseFees);

        expect(result.trendAdjustedProjectedCostXlm).toBeLessThan(result.flatProjectedCostXlm);
        expect(result.trendMultiplier).toBeLessThan(1);
        expect(result.feeTrend).toBe("falling");
    });

    it("returns stable trend with equal projection when no fee samples are provided", () => {
        const result = calculateTrendAdjustedProjection(2.0, 30);

        expect(result.flatProjectedCostXlm).toBe(2);
        expect(result.trendAdjustedProjectedCostXlm).toBeCloseTo(2, 7);
        expect(result.trendMultiplier).toBeCloseTo(1, 7);
        expect(result.feeTrend).toBe("stable");
    });

    it("returns stable trend with a single fee sample", () => {
        const result = calculateTrendAdjustedProjection(1.5, 10, [150]);

        expect(result.trendMultiplier).toBeCloseTo(1, 7);
        expect(result.feeTrend).toBe("stable");
    });

    it("computes the flat projection correctly from period cost", () => {
        const result = calculateTrendAdjustedProjection(3, 10, [100, 100, 100, 100]);

        expect(result.flatProjectedCostXlm).toBe(9);
    });

    it("handles an odd number of fee samples by dropping the median", () => {
        const recentBaseFees = [100, 100, 100, 150, 200, 200, 200];
...
describe("getExtensionCosts with trend projection", () => {
    it("includes trendProjection in result when fee samples are provided", () => {
        recordExtension(db, {
            contract_id: CONTRACT_ID,
            contract_entry_id: entryId,
            old_ttl_ledgers: 1000,
            new_ttl_ledgers: 50000,
            tx_hash: "hash-trend-1",
            cost_xlm: 0.5,
            executed_at_ledger: 12345,
        });
        ...
        const result = getExtensionCosts(db, CONTRACT_ID, {
            period: 30,
            recentBaseFees: [100, 100, 100, 100, 200, 200, 200, 200],
        });

        expect(result.success).toBe(true);
        if (!result.success) return;

        expect(result.data.trendProjection).toBeDefined();
        expect(result.data.trendProjection!.flatProjectedCostXlm).toBeCloseTo(0.5, 7);
        expect(result.data.trendProjection!.trendAdjustedProjectedCostXlm).toBeGreaterThan(
            result.data.trendProjection!.flatProjectedCostXlm,
        );
        expect(result.data.trendProjection!.feeTrend).toBe("rising");
    });

    it("omits trendProjection when no fee samples are provided", () => {
        recordExtension(db, {
            contract_id: CONTRACT_ID,
            contract_entry_id: entryId,
            old_ttl_ledgers: 1000,
            new_ttl_ledgers: 50000,
            tx_hash: "hash-no-trend",
            cost_xlm: 0.5,
            executed_at_ledger: 12345,
        });
        ...
        expect(result.data.trendProjection).toBeUndefined();
    });

    it("shows stable trend when all fee samples are equal", () => {
        recordExtension(db, {
            contract_id: CONTRACT_ID,
            contract_entry_id: entryId,
            old_ttl_ledgers: 1000,
            new_ttl_ledgers: 50000,
            tx_hash: "hash-stable-trend",
            cost_xlm: 1.0,
            executed_at_ledger: 12345,
        });
        ...
        const result = getExtensionCosts(db, CONTRACT_ID, {
            period: 30,
            recentBaseFees: [100, 100, 100, 100, 100, 100, 100, 100],
        });

        expect(result.success).toBe(true);
        if (!result.success) return;

        expect(result.data.trendProjection!.feeTrend).toBe("stable");
        expect(result.data.trendProjection!.trendAdjustedProjectedCostXlm).toBeCloseTo(
            result.data.trendProjection!.flatProjectedCostXlm,
            7,
        );
    });

    it("omits trendProjection when no extensions exist even with fee samples", () => {
        const result = getExtensionCosts(db, CONTRACT_ID, {
            period: 30,
            recentBaseFees: [100, 200],
        });
