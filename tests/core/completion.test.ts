...
describe("existing completion generators remain unchanged", () => {
  ...
  it("TOP_LEVEL_COMMANDS, ALERTS_SUBCOMMANDS, and CHANNELS_SUBCOMMANDS remain accessible", () => {
    expect(TOP_LEVEL_COMMANDS).toContain("completion");
    expect(ALERTS_SUBCOMMANDS).toContain("add");
    expect(CHANNELS_SUBCOMMANDS).toContain("add");
  });
});
