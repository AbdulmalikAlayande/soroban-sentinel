...
describe("Fleet Core - applyGuardPolicyByTag", () => {
    ...
    it("applies a policy to a tag with 3 matching contracts identically on all 3", async () => {
        ...
        // Compare only the policy fields that should be identical across contracts —
        // id/contract_id/created_at legitimately differ per row.
        const policyFields = (p: typeof p1) => ({
            enabled: p?.enabled,
            target_ttl_ledgers: p?.target_ttl_ledgers,
            extend_when_below_ledgers: p?.extend_when_below_ledgers,
            keypair_public: p?.keypair_public,
            keypair_source: p?.keypair_source,
        });
        expect(policyFields(p2)).toEqual(policyFields(p1));
        expect(policyFields(p3)).toEqual(policyFields(p1));
    });
